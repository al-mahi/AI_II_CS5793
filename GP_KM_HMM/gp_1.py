...
if __name__ == "__main__":
    ...
    # num = 0
    # for sigma in np.logspace(-1.3, -.7, num=100):
    ...
    opt_mse = np.inf
    opt_sigma = np.nan
    for sigma in np.logspace(-1.3, -.7, num=num_sigmas):
        num = 0
    ...
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.title("GP with $\sigma_*$={:.6f}".format(opt_sigma))
    plt.savefig("fig_k1.png", format='png')
    plt.show()
